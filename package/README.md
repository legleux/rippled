...
`xrpld` and the `validator-keys` tool.
...
uses. The image tag is derived from `linux.json` so you don't need to hardcode
a SHA.
...
of `rpmbuild` / `dpkg-buildpackage` is present and `validator-keys` is enabled;
`build_pkg.sh` then infers the package format from the host's package manager.
...
`stage_common()` to copy the binaries, config files, and shared support files
into the staging area, and invokes the platform build tool.

### RPM

1. Creates the standard `rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}` tree inside the build directory.
2. Copies `xrpld.spec` and all source files (binaries, configs, service files) into `SOURCES/`.
...
```bash
# DEB
dpkg-deb -c debbuild/*.deb | grep -E 'validator-keys|systemd|sysusers|tmpfiles'
lintian -I debbuild/*.deb

# RPM
rpm -qlp rpmbuild/RPMS/x86_64/*.rpm | grep validator-keys
```
