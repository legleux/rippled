...
def generate_strategy_matrix(all_configs: bool, config: Config) -> list:
    ...
    for architecture, os, build_type, cmake_args in itertools.product(config.architecture, config.os, config.build_type, config.cmake_args):
        ...
        if not all_configs and not package:
            # Debian:
        ...
        # if os['distro_name'] in ('debian', 'rhel') and compiler == 'gcc-12':
        #     package = True
        ...
        if build_type == 'Release' and not package:
            cmake_args = f'{cmake_args} -Dwerr -Dassert=ON'
        else:
            cmake_args = f'{cmake_args}'
        ...
        ####
        # config_name_ = [field for field in [
        #     os['distro_name'],
        #     os['distro_version'],
        #     os['compiler_name'],
        #     os['compiler_version'],
        #     (arch := architecture['platform'].split('/')[-1]),
        #     build_type.lower(),
        #     'unity' if '-Dunity=ON' in cmake_args else '',
        #     ] if field != '' ]
        # distro_name = os['distro_name']:
        # distro_version = os['distro_version']:
        #     config_name_.append(distro_version)
        #     distro = f"{os['distro_name']}-{os['distro_version']}"
        # config_name_.append(os.get('compiler_name'))
        # if compiler_version := os.get('compiler_version'):
        #     config_name_.append(compiler_version)
        #     compiler = f"{os['compiler_name']}-{os['compiler_version']}"
        # config_name_.append(arch := architecture['platform'].split("/")[-1])
        # config_name_.append(build_type.lower())
        # config_name_.append('unity' if '-Dunity=ON' in cmake_args else None)

        if '-Dunity=ON' in cmake_args:
            config_name += '-unity'
            # config_name_ += 'unity' if '-Dunity=ON' in cmake_args else None
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--all-configs', help='Set to generate all configurations (generally used when merging a PR) or leave unset to generate a subset of configurations (generally used when committing to a PR).', action="store_true")
    ...
    if args.config is None or args.config == '':
        matrix += generate_strategy_matrix(args.all_configs, read_config(THIS_DIR / "linux.json"))
        matrix += generate_strategy_matrix(args.all_configs, read_config(THIS_DIR / "macos.json"))
        matrix += generate_strategy_matrix(args.all_configs, read_config(THIS_DIR / "windows.json"))
    else:
        matrix += generate_strategy_matrix(args.all_configs, read_config(args.config))

    # Generate the strategy matrix.
    # print(f'matrix={json.dumps({"include": matrix})}')
    print(f'matrix={json.dumps({"include": matrix}, indent=2)}')
